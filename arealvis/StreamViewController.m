...
- (void)viewWillAppear:(BOOL)animated{
    self.results= [[xmlParser new] withXPath:@"/config/channel/framesrc/libav/url"];
...
{
    static NSString *CellIdentifier = @"CannelCell";
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if([[segue identifier] isEqualToString:@"getStream"]){
        NSIndexPath *myIndex = [self.tableView indexPathForSelectedRow];
        DDXMLElement *channel = [results objectAtIndex:myIndex.row];
        DDXMLNode *child = [channel childAtIndex:0];
        
        GetStreamViewController *detailViewController = (GetStreamViewController *)[segue destinationViewController];
        
        detailViewController.url =  [child stringValue];
